...
public class InvContentAdapter extends CircleScrollerRecyclerAdapter<InvContentItemBean> {

    public InvContentAdapter(Context context) {
        super(context, R.layout.item_class_detail_left);
    }

    @Override
    public void convert(RecyclerHolder holder, InvContentItemBean invContentItemBean, final int position) {
        TextView name = holder.getView(R.id.tv);
        name.setText(invContentItemBean.getContent());
        if(invContentItemBean.getSelected()){
