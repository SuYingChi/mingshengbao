...
public class DrawHookView extends View {
    ...
    private final int drawInterval = 6;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //先等圆弧画完，才话对勾
        if(progress >= 100) {
            if(line1_x < radius / 2.5) {
                line1_x+=drawInterval;
                line1_y+=drawInterval;
            }
            //画第一根线
            canvas.drawLine(center1, center, center1 + line1_x, center + line1_y, paint);

            if (line1_x <= radius / 2.5+drawInterval) {
                line2_x = line1_x;
                line2_y = line1_y;
                line1_x+=drawInterval;
                line1_y+=drawInterval;
            }
            if (line1_x >= radius / 2.5+drawInterval&&line2_x<=radius+getWidth()/7) {
                line2_x+=drawInterval;
                line2_y-=drawInterval;
            }
            //画第二根线
