...
public abstract class MyHaveHeadAndFootRecyclerAdapter<T> extends RecyclerView.Adapter<RecyclerHolder> {
    ...
    public MyHaveHeadAndFootRecyclerAdapter(Context context, int layoutId) {
        this.context = context;
        this.layoutId = layoutId;
        this.monetary_unit = context.getResources().getString(R.string.monetary_unit);
    }

    public MyHaveHeadAndFootRecyclerAdapter(Context context, int layoutId, List<T> datas) {
        this.context = context;
        this.layoutId = layoutId;
        this.datas = datas;
        this.monetary_unit = context.getResources().getString(R.string.monetary_unit);
    ...
    public void setFoot_layoutId(int foot_layoutId) {
    ...
    public void onBindViewHolder(RecyclerHolder holder, int position) {
        if (getItemViewType(position) == Integer.MIN_VALUE) {
            ...
            convert(holder, datas.get(datas.size() - 1), position);
        } else {
