...
public class WuliuCompanyAdapter extends CircleScrollerRecyclerAdapter<WuliuCompanyItemBean> {

    public WuliuCompanyAdapter(Context context) {
        super(context, R.layout.item_class_detail_left);
    }

    @Override
    public void convert(RecyclerHolder holder, WuliuCompanyItemBean itemBean, final int position) {
        TextView name = holder.getView(R.id.tv);
        name.setText(itemBean.getContent());
